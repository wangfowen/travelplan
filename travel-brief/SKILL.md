...
- Save the run record to `runs/<city-slug>-<YYYY-MM-DD>/`: copy the dossier files in, and build `decisions.md` by concatenating `decisions/*.md` in section order — append any research gaps or source failures the drafters reported. This is what `/travel-summary` audits after the trip.
