...
- **The cocktail-bar bar should bite harder, or the section be dropped.** "Interesting — doing things no one else is doing" is already the written bar, but venues clearing it on paper (a gimmick format, a large from-memory menu) can still be ordinary in the room. Watching whether the section earns its place at all for this traveler, who reads bars as destinations only when something else is on offer. Would edit: `food.md` → Cocktail bars and breweries. Sightings: Vilnius 2026-08-13 (Alchemikas and Šnekutis are both of the trip's two dislikes; *"don't care about going to a bar for the sake of a bar"* — §5).
